...
<head>
	...
	<script type="text/javascript">
		$(function() {
			...
			$(".jian").click(function() {
				...
				if (quantity == 1) {
					...
					ajaxupdatequantity(cartItemId, quantity - 1);
				}
			});
			$(".jia").click(function() {
				var cartItemId = $(this).attr("id").substring(0, 32);
			...
			function ajaxupdatequantity(cartItemId, quantity) {
				$.ajax({
					url : "/Books/servlet/CartServlet?method=updateQuantityByCartItemId",
					data : "cartItemId=" + cartItemId + "&quantity=" + quantity,
					dataType : "json",
					success : function(result) {
						sendUpdate(cartItemId, quantity);
					}
		...
		function batchDeleteCartItems() {
			...
			window.location.href = "/cartItemAction/deletelistCarItem.do?cartItemIds="
					+ ids;
		}
		function accounts(){
			var ids=[];
			...
			window.location.href="/cartItemAction/accountsCartByIds.do?cartItemIds="
					+ ids;
		}
		// 显示合计
		function showTotal() {
		...
		<c:otherwise>
			<table width="95%" align="center" cellpadding="0" cellspacing="0">
					...
					<tr align="center">
						...
						<td align="left" width="70px"><a class="linkImage"
							href="<c:url value='/jsps/book/desc.jsp'/>"> <!-- 这个image标签是购物车中添加商品的图片 -->
								...
								src="/${cartItem.book.image_b}" /></a></td>
						...
						<td><a
							href="/cartItemAction/delectCartItem.do?cartItemId=${cartItem.cartItemId}">删除</a></td>
					</tr>
				</c:forEach>
				<tr>
				...
				<tr>
					...
					<a
						href="javascript:accounts()" id="jiesuan"
						class="jiesuan"></a></td>
				</tr>
			</table>
			<form id="form1" action="/cartItemAction/accountsCartByIds.do"
				method="post">
